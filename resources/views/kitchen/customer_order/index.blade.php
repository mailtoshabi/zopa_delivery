...
<div class="row">
    ...
    <ul class="nav nav-tabs">
        <li class="nav-item">
          <a class="nav-link @if($is_active==Utility::ITEM_INACTIVE) active @endif" @if($is_active==Utility::ITEM_INACTIVE)aria-current="page"@endif href="{{ route('kitchen.orders.index','status='.encrypt(Utility::ITEM_INACTIVE)) }}">Pending to add Wallet <span class="badge rounded-pill bg-soft-danger text-danger float-end">{{ $count_new }}</span></a>
        </li>
        <li class="nav-item">
          <a class="nav-link @if($is_active==Utility::ITEM_ACTIVE) active @endif" @if($is_active==Utility::ITEM_ACTIVE)aria-current="page"@endif href="{{ route('kitchen.orders.index','status='.encrypt(Utility::ITEM_ACTIVE)) }}">Added to Wallet</a>
        </li>
        <li class="nav-item">
            <a class="nav-link text-danger @if($is_active==Utility::STATUS_NOTPAID) active @endif" @if($is_active==Utility::STATUS_NOTPAID)aria-current="page"@endif href="{{ route('kitchen.orders.index','status='.encrypt(Utility::STATUS_NOTPAID)) }}">UnPaid <span class="badge rounded-pill bg-soft-danger text-danger float-end">{{ $count_not_paid }}</span></a>
          </li>
      </ul>
    </div>
</div>
                                                    ...
                                                    <div class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-menu-end">
                                                            @if($customer_order->status == Utility::ITEM_INACTIVE)
                                                                <li>
                                                                <a onclick="return confirm('Are you sure to Activate & Mark as Paid?')" class="dropdown-item" href="{{ route('kitchen.orders.activate', [encrypt($customer_order->id), 'paid']) }}">
                                                                    <i class="fas fa-credit-card font-size-16 text-success me-1"></i> Activate With Payment
                                                                </a>
                                                                </li>

                                                                <li>
                                                                <a onclick="return confirm('Are you sure to Activate without Payment?')" class="dropdown-item" href="{{ route('kitchen.orders.activate', [encrypt($customer_order->id), 'unpaid']) }}">
                                                                    <i class="fas fa-toggle-on font-size-16 text-success me-1"></i> Activate Without Payment
                                                                </a>
                                                                </li>
                                                            @endif
                                                            <li><a class="dropdown-item" onclick="return confirm('Are you sure to make the change?')" href="{{ route('kitchen.orders.changePayment',encrypt($customer_order->id))}}">{!! $customer_order->is_paid?'<i class="fas fa-power-off font-size-16 text-danger me-1"></i> Mark Un Paid':'<i class="fas fa-circle-notch font-size-16 text-primary me-1"></i> Mark Paid'!!}</a></li>
                                                        </ul>
                                                    </div>
                                                </td>
                                        </tr>
