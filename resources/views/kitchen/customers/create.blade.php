...
@endcomponent
<div class="row">
    <form method="POST" action="{{ isset($customer)? route('kitchen.customers.update',encrypt($customer->id)) : route('kitchen.customers.store')  }}" enctype="multipart/form-data">
        @csrf
        @if (isset($customer))
            @method('PUT')
        @endif
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="mb-3 required">
                                ...
                                <input type="hidden" id="kitchen_id" name="kitchen_id" value="{{ encrypt(auth('kitchen')->id()) }}" >
                        ...
                        <div class="col-sm-6">
                            ...
                            <div class="mb-3 required">
                                ...
                                <select id="language" name="language" class="form-control select2">
                                    <option value="en" {{ isset($customer) && $customer->language === 'en' ? 'selected' : '' }}>English</option>
                                    <option value="ml" {{ isset($customer) &&  $customer->language === 'ml' ? 'selected' : '' }}>മലയാളം</option>
                                </select>
...
<script>
    ...
    function getdistrict(val,d_id) {
        ...
        $.ajax({
            type: "POST",
            url: "{{ route('kitchen.list.districts') }}",
            data:formData,
