        ...
        @foreach ($customers as $customer)
            <tr>
                <td>
                    ...
                    @endif
                    <a href="{{ route('kitchen.customers.edit',encrypt($customer->id)) }}" class="">{{ $customer->name }}</a>
                    </td>
                    ...
                    <td>
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                <li><a class="dropdown-item" href="{{ route('kitchen.customers.edit',encrypt($customer->id))}}"><i class="mdi mdi-pencil font-size-16 text-success me-1"></i> Edit</a></li>
                                ...
                                <form id="form_delete_{{ $loop->iteration }}" method="POST" action="{{ route('kitchen.customers.destroy',encrypt($customer->id))}}">
                                    @csrf
                                    <input type="hidden" name="_method" value="DELETE" />
                                </form> --}}
                                    <li><a class="dropdown-item" onclick="return confirm('Are you sure to make the change?')" href="{{ route('kitchen.customers.changeStatus',encrypt($customer->id))}}">{!! $customer->is_approved?'<i class="fas fa-power-off font-size-16 text-danger me-1"></i> Suspend':'<i class="fas fa-circle-notch font-size-16 text-primary me-1"></i> Activate'!!}</a></li>
                                <li><a class="dropdown-item" href="{{ route('kitchen.customers.show',encrypt($customer->id))}}"><i class="fa fa-eye font-size-16 text-success me-1"></i> Details</a></li>
                            </ul>
                        </div>
                    </td>
            </tr>
