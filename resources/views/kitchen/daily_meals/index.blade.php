                            ...
                            @if($mealtype ==1)
                                <div class="col-md-8 text-end">
                                    <div class="d-inline-block me-2">
                                        <form action="{{ route('kitchen.daily_meals.mark.all.delivered') }}" method="POST" onsubmit="return confirm('Are you sure to mark all as delivered?')">
                                            @csrf
                                    ...
                                    <div class="d-inline-block me-2">
                                        <form action="{{ route('kitchen.daily_meals.undo.delivered') }}" method="POST" class="d-inline-block" onsubmit="return confirm('Are you sure to undo all delivered meals for today?')">
                                            @csrf
                                    ...
                                    <div class="d-inline-block">
                                        <form action="{{ route('kitchen.daily_meals.generate') }}" method="POST" onsubmit="return confirm('Are you sure to generate daily meals?')">
                                            @csrf
                                    ...
                                    {{-- <div class="d-inline-block">
                                        <form action="{{ route('kitchen.daily_meals.generate.institution') }}" method="POST" onsubmit="return confirm('Are you sure to generate daily meals for Institution?')">
                                            @csrf
                            ...
                            @if($mealtype !=1)
                                <form method="GET" class="row g-3 mb-3">
                                    ...
                                    <div class="col-auto">
                                        ...
                                        <a href="{{ route('kitchen.daily_meals.index') }}" class="btn btn-outline-secondary btn-sm">Reset</a>
                                    </div>
                                </form>
                            @endif
                                                        ...
                                                        <ul class="dropdown-menu dropdown-menu-end">
                                                            ...
                                                            @if($meal->status && $meal->date->lte(\Carbon\Carbon::today()))
                                                                ...
                                                                @else
                                                                    ...
                                                                    <li>
                                                                        <form action="{{ route('kitchen.daily_meals.changeDelivery', encrypt($meal->id)) }}" method="POST" onsubmit="return confirm('Mark as Delivered?')">
                                                                            @csrf
...
@foreach ($dailyMeals as $meal)
    ...
    <div class="modal fade" id="undeliverModal{{ $meal->id }}" tabindex="-1" aria-labelledby="undeliverModalLabel{{ $meal->id }}" aria-hidden="true">
        <div class="modal-dialog">
            <form method="POST" action="{{ route('kitchen.daily_meals.changeDelivery', encrypt($meal->id)) }}">
                @csrf
