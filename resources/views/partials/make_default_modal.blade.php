...
@if($meal_wallets->isNotEmpty())
    <div id="mealWalletList" style="display: none;">
        @foreach ($meal_wallets as $meal_wallet)
            @if($meal_wallet->quantity>0 && $meal_wallet->status)
                <div class="row d-flex align-items-center mb-3" style="width: 100%;">
                    <div class="col-sm-12 col-md-6 mb-3 mb-md-0">
                        <div class="d-flex align-items-center">
                            ...
                            <span class="me-2 {{ $meal_wallet->status ? 'text-dark' : 'text-muted' }}">{{ $meal_wallet->walletGroup->display_name }}</span>
                            @if($meal_wallet->is_on)
...
<script>
    ...
    function bindMakeDefaultButtons() {
        $('.make-default-btn').off('click').on('click', function () {
            ...
            $.ajax({
                url: "{{ route('front.wallet.make-default') }}",
                type: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                    wallet_id: walletId
